...
if [ ! -f "$INSTALLED_FILE_PATH" ]; then
    ...
    tar -xvzf built.tar.gz -C /home/bun/app > /dev/null 2>&1
...
bun run --bun --env-file=.env ./dist/main.js
...
# cp ~/production/docker-compose/services/cuce/cuce-backend.env ~/production/repos/CUCE_BE/.env && docker build --no-cache --tag=cuce-backend:production --file ~/production/repos/CUCE_BE/Dockerfile --progress=plain /root/production/repos/CUCE_BE (Always do this after any repo update of the services)
